...
100   if (ierr .ne. 0) then
   ...
   subroutine read_atmosphere_nc_ls(infile, outputflag, atm_scenario, meta, ierr, meteo_errors)
      !*** Input
      character(stringlen), intent(in) :: infile
      real(double), dimension(4), intent(in), optional :: meteo_errors
      integer, intent(in) :: outputflag
      !*** Output
      type(atmospheric_scenario), intent(out) :: atm_scenario
...
!-----------------------------------------------------
      ...
      if (outputflag >= 2) then
         call writelog('*** Start of READ_ATMOSPHERE_NC_LS ***', 1)
      ...
      if (ierr .ne. 0) then
         ierr = ierr_open
         call writelog('READ_ATMOSPHERE_NC_LS: error opening file: '//trim(meteo_file), 6)
         return
      end if
...
100   if (ierr .ne. 0) then
         if (outputflag >= 2) then
            write (message, '(a)') 'READ_ATMOSPHERE_NC_LS: Error opening/reading meteo_file '//trim(meteo_file)
            call writelog(message, 6)
         end if
         return
      end if
      ...
      if (outputflag >= 2) then
         call writelog('*** End of READ_ATMOSPHERE_NC_LS ***', 1)
      end if

   end subroutine read_atmosphere_nc_ls
